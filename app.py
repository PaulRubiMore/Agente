    ...
    if ejecutar or "cron" not in st.session_state:
        with st.spinner("⚙️ Ejecutando modelo de optimización..."):
            try:
                ...
                dividir = dividir_especialidades(cron)
                matriz_tecnicos = optimizar_tecnicos_turnos(cron)
                st.session_state.update({"cron": cron, "cs": cs, "tecnicos": df_tecnicos, "tecnicos_ot": df_tecnicos_ot, "dividir": dividir_ot, "matriz_tecnicos": matriz_tecnicos})
